...
class TrollBrain:
    def __init__(self, data_path="data/memoria_ia.jsom"):
    ...
    def aprender(self, frase, categoria):
        """Descompone la frase y guarda las palabras en la categoría correspondiente"""
        palabras =  frase.lower().split()

        for palabra in palabras:
            #Filtro básico: ignorar palabreas de menos de 2 letras
            if len(palabra) > 2:
                if categoria == "toxico":
                    #Si existe suma1, si no existe empieza en 0 y suma 1
    ...
    def predecir(self, frase):
        ...
        score_toxico = 0
        score_pro = 0
        detalles = [] #Para explicar por que tomo la decision
        palabras = frase.lower().split
